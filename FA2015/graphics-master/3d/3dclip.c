...
double hither = 0, yon, color[3], intersect[3];
...
void intersection(double startx, double endx, double starty, double endy,
                  double startz, double endz, int planenumber){
  double tval,t,sum;

  sum=0;

  if(planenumber==0){//hither plane
    tval=endz-startz;
    sum=startz-hither;
    t=sum/(tval*-1);
    intersect[0]=startx+(t*(endx-startx));
    intersect[1]=starty+(t*(endy-starty));
    intersect[2]=startz+(t*(endz-startz));
    return;
  }

  if(planenumber==1){//yon plane
    tval=endz-startz;
    sum=startz-yon;
    t=sum/(tval*-1);
    intersect[0]=startx+(t*(endx-startx));
  ...
  if(planenumber==2){//top plane
    tval=-(endz-startz)+(endy-starty);
    sum=-startz+starty;
    t=sum/(tval*-1);
    intersect[0]=startx+(t*(endx-startx));
    intersect[1]=starty+(t*(endy-starty));
  ...
  if(planenumber==4){//left plane
    tval=-(endx-startx)+(endz-startz);
    sum=-startx+startz;
    t=sum/(tval*-1);
    intersect[0]=startx+(t*(endx-startx));
    intersect[1]=starty+(t*(endy-starty));
...
int clip_the_image(double *xclip,double *yclip,double *zclip, int size){
  double solx[1000],soly[1000],solz[1000],centerVal,toVal,fromVal,fv,tv,mid;
  int i,j,r,m,shape,p,planenumber;

  mid = (hither+yon)/2;

  // 0=z-hither  hither plane #0
  // 0=z-yon     yon plane #1
  // 0=y-z       top #2
  // 0=-y-z      bottom #3
  // 0=-x+z      left #4
 ...
 for(planenumber=0;planenumber<6;planenumber++){ //plane numbers
    ...
    shape=0;
     for(i=0;i<size;i++){
      ...
      if(planenumber==0){
       fromVal=zclip[i]-hither;//start
       toVal = zclip[m]-hither;//end
       centerVal=mid-hither;
       }


     else if(planenumber==1){
       fromVal=zclip[i]-yon;//start
       toVal = zclip[m]-yon;//end
       centerVal=mid-yon;
       }


     else if(planenumber==2){
       fromVal=yclip[i]-zclip[i];//start
       toVal = yclip[m]-zclip[m];//end
       centerVal = -mid ;
       }
       ...
       fromVal= zclip[i]-xclip[i];//start
       toVal = zclip[m]-xclip[m];//end
       centerVal=mid ;
       }
       ...
       //in to out
       else if ((fv > 0) && (tv < 0)) { //intersection
        intersection(xclip[i],xclip[m],yclip[i],yclip[m],zclip[i],zclip[m],planenumber);
        solx[shape]= intersect[0];
        soly[shape]= intersect[1];
        solz[shape]= intersect[2];
        shape++;
       }
       //out to in
       else if ((fv < 0) && (tv > 0)) { //intersection and next in point
        intersection(xclip[i],xclip[m],yclip[i],yclip[m],zclip[i],zclip[m],planenumber);
        solx[shape]= intersect[0];
        soly[shape]= intersect[1];
        solz[shape]= intersect[2];
...
double intensity(double *polyx, double *polyclip, double *polyz){
  double v[3],w[3],e[3],l[3],norm[3],reflect[3],angle, specularity;

  v[0] = polyx[1] - polyx[0];
  v[1] = polyclip[1] - polyclip[0];
  v[2] = polyz[1] - polyz[0];  

  w[0] = polyx[2] - polyx[0];
  w[1] = polyclip[2] - polyclip[0];
  w[2] = polyz[2] - polyz[0];

  l[0] = lightx - polyx[0];
  l[1] = lighty - polyclip[0];
  l[2] = lightz - polyz[0];
...
void draw_all_objects(THING *allpolys, int numthings){
  ...
  for (i=0; i < numthings; i++){
    ...
    double polyx[size], polyclip[size], polyz[size];
    for(j=0; j < size; j++){
      int side = con[onum][polyn][j];
      polyx[j]= x[onum][side];
      polyclip[j]= y[onum][side];
      polyz[j]= z[onum][side];
    }
    size = clip_the_image(polyx,polyclip,polyz,size);

    for (j = 0; j< size; j++){
      xb = polyx[j]/polyz[j];
      yb = polyclip[j]/polyz[j];
    
      xprime[j] = (halfangle*xb) + 300;
      yprime[j] = (halfangle*yb) + 300;
    }
    double lmodel = intensity(polyx, polyclip, polyz);
